...
# try: 
#     from flash_attn.tests.test_flasn_attn import convert_flash_attn_S_to_softmax, normalize_flash_attn_S
# except:
#     print("IMPORT ERROR")
#     convert_flash_attn_S_to_softmax = None
#     normalize_flash_attn_S = None
    ...
    def forward(self, qkv, causal=None, cu_seqlens=None, max_seqlen=None):
        ...
        if unpadded:
            ...
            return flash_attn_varlen_qkvpacked_func(
                ...
                cu_seqlens,
                max_seqlen,
                self.drop.p if self.training else 0.001, # WARNING-- this is to return the attention
...
class MHA(nn.Module):
    ...
    def __init__(
        ...
        inner_attn_cls = (
            partial(FlashSelfAttention, alibi_slopes=alibi_slopes, window_size=window_size, return_attn_probs=return_attn_probs)
            if use_flash_attn
            else SelfAttention
        )
        inner_cross_attn_cls = (
            partial(FlashCrossAttention, alibi_slopes=alibi_slopes, window_size=window_size, return_attn_probs=return_attn_probs)
            if use_flash_attn
            else CrossAttention
        )
            ...
            if (
                ...
                if inference_params is None:
                    if not self.checkpointing:
                        if self.use_flash_attn==False and self.return_attn_probs:
                            context, attn = self.inner_attn(qkv, **kwargs)
                        elif self.return_attn_probs and self.use_flash_attn:
                            context, lse, S_dmask = self.inner_attn(qkv, **kwargs)
